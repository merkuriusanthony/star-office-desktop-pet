...
fn spawn_backend(root: &PathBuf) -> Option<Child> {
    // Skip spawning a local backend entirely when pointed at a remote
    // Star Office instance (e.g. dashboard.claireantonia.id on the NAS).
    if std::env::var("STAR_REMOTE_BACKEND").is_ok() {
        eprintln!("ℹ️ STAR_REMOTE_BACKEND set — skipping local backend spawn");
        return None;
    }
